...
import { Package, Search, Grid, List, ChevronRight, Layers, PlayCircle } from 'lucide-react';
...
export default function AdLabPage() {
  ...
  const { data: packCounts = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      (data || []).forEach((pack: { produto_id: string; status: string }) => {
        if (!counts[pack.produto_id]) {
          counts[pack.produto_id] = { produto_id: pack.produto_id, total: 0, validados: 0 };
        }
        counts[pack.produto_id].total++;
        if (pack.status === 'validado') {
          counts[pack.produto_id].validados++;
        }
      });
  ...
  const { data: anuncioCounts = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      (packs || []).forEach((pack: { id: string; produto_id: string }) => {
        packToProduct[pack.id] = pack.produto_id;
      });
