...
import { CreateArquitetoDialog } from '../arquitetos/CreateArquitetoDialog';
import { EditArquitetoDialog } from '../arquitetos/EditArquitetoDialog';
...
export function RegistroCreators() {
  const { user } = useAuth();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast.error('Erro ao excluir creator');
      console.error(error);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-display font-bold">Registro de Creators</h2>
        <CreateArquitetoDialog onSuccess={loadCreators} />
      </div>
          ...
          <TableBody>
            {creators.length === 0 ? (
              ...
              creators.map((creator) => (
                ...
                >
                  <TableCell className="font-medium">{creator.nome_completo}</TableCell>
                  <TableCell className="text-primary">{creator.arroba_principal}</TableCell>
                  <TableCell className="text-muted-foreground">{creator.email_contato}</TableCell>
                  <TableCell className="text-muted-foreground">{creator.telefone_contato}</TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Badge variant="outline" className={getStatusColor(creator.status_arquiteto)}>
                      {creator.status_arquiteto}
                    </Badge>
                  </TableCell>
                  <TableCell className="font-mono text-sm">{creator.cupom_exclusivo}</TableCell>
